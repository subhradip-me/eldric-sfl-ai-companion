// Groq chat: LLM explains, never calculates (design §20)
...
export async function chat(message, context) {
  ...
  content = content.replace(/<think>[\s\S]*?<\/think>/g, "").trim(); // strip reasoning blocks
  if (!content) throw new Error("Groq returned an empty answer — check GROQ_MODEL in .env (try llama-3.3-70b-versatile).");
  return content;
}
